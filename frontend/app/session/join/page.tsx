...
export default function JoinSessionPage() {
  ...
  const startCamera = async () => {
    try {
      ...
      if (videoRef.current) {
        ...
        setTimeout(() => {
          const mockToken = "MOCK" + Math.random().toString(36).substring(2, 8).toUpperCase()
  ...
  const joinSession = async () => {
    ...
    try {
      ...
      // Mock validation - accept any token for demo
      if (sessionToken.length >= 6) {
        router.push(`/auth/chat/${sessionToken}`)
      } else {
        setError("Invalid session token. Please check and try again.")
      }
    } catch (err) {
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <TabsContent value="token" className="space-y-4 mt-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="token"
                    placeholder="Enter session token (e.g., ABC123XYZ)"
                    value={sessionToken}
                    onChange={(e) => setSessionToken(e.target.value.toUpperCase())}
                    className="font-mono"
                  />
                  <p className="text-sm text-muted-foreground">Get this token from the session host</p>
                </div>
