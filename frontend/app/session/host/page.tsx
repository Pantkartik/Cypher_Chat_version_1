...
export default function HostSessionPage() {
  ...
  const generateSession = async () => {
    ...
    setSessionToken(token.toUpperCase())

    // Mock QR code URL - in real app this would be generated server-side
    setQrCodeUrl(`/placeholder.svg?height=200&width=200&query=QR code for session ${token}`)
          ...
          <div className="space-y-6">
                ...
                <CardContent>
                  <div className="flex flex-col items-center space-y-4">
                    <div className="p-4 bg-white rounded-lg shadow-sm border">
                      <img src={qrCodeUrl || "/placeholder.svg"} alt="Session QR Code" className="w-48 h-48" />
                    </div>
            ...
            {sessionToken && (
              <Alert>
                ...
                <AlertDescription>
                  Share the session token or QR code with participants. The session will remain active until you close
                  it.
                </AlertDescription>
              </Alert>
            )}
